...
#   ./client/build_app.sh --server http://dash      # bake in the dashboard URL
#   ./client/build_app.sh --desktop                 # also install a .desktop launcher
#
# Baking in --server is what makes the result zero-configuration: the app connects on
# launch and the user never types a URL. Combined with the server's identity-based
# access (no token in most deployments), double-clicking is the whole setup.
#
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --server)  DEFAULT_SERVER="${2:-}"; shift 2 ;;
    --t4-url)  DEFAULT_T4="${2:-}"; shift 2 ;;
    --desktop) INSTALL_DESKTOP=1; shift ;;
    *) echo "unknown option: $1" >&2; exit 2 ;;
  esac
done

# Generated, not tracked: the baked-in defaults for this particular build.
DEFAULTS_PY="$REPO_ROOT/client/_defaults.py"
if [[ -n "$DEFAULT_SERVER" || -n "$DEFAULT_T4" ]]; then
  cat > "$DEFAULTS_PY" <<EOF
"""Build-time defaults. Generated by client/build_app.sh; do not edit or commit."""

DEFAULT_SERVER = "${DEFAULT_SERVER}"
DEFAULT_T4_BASE_URL = "${DEFAULT_T4}"
EOF
  echo "==> baking in server default: ${DEFAULT_SERVER:-<none>}"
else
  rm -f "$DEFAULTS_PY"
fi
...
for ASSET in "" workflow explorer viewer tlr bbox_theme.js bbox_explorer.css events.js tlr_viewer.css tlr_api.js tlr_viewer.js; do
...
# The TLR analysis page needs pandas-backed routes from lib/; an empty workspace still answers.
curl -sf -m 15 -X POST "http://127.0.0.1:$SMOKE_PORT/api/tlr_dirs" -d '{}' \
  | grep -q '"items"' || fail "/api/tlr_dirs did not answer"

[[ -n "$SMOKE_PID" ]] && kill "$SMOKE_PID" 2>/dev/null
echo "    ok (health, 10 assets, duckdb route, tlr route)"
